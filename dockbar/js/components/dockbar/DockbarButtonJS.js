var DockbarButton = React.createClass({
    ...
    clickHandler: function () {
        ...
        if (hasClosed) {
            ...
            this.referencedWindows = _.reject(this.referencedWindows, function (a) {
                return a.id === componentId;
            });
    ...
    showWindows: function (callback) {
        ...
        windows.forEach(function (window) {
            ...
            if (window.windowName) {
                ...
                existingWindow = _.find(this.referencedWindows, function (win) {
                    return win.name === window.windowName;
                });
            }
            ...
            if (existingWindow == null) {
                ...
                finWindow.addEventListener("closed", function (win) {
                    ...
                    context.referencedWindows = _.reject(context.referencedWindows, function (a) {
                        return a.fin.name === win.name;
                    });
                    console.log("remaining windows: " + context.referencedWindows.length);
                    context.props.onWindowClosed(win.name, componentId);
                    if (context.referencedWindows.length == 0) context.props.onAllWindowClosed(componentId);
                });
                ...
                finWindow.show(true, function (a) {
                    finWindow.moveTo(left, top, function () {
                        counter++;
                        if (counter == windows.length && callback) callback(componentId);
                    });
    ...
    render: function () {
        ...
        return React.createElement('img', { id: this.props.componentId, style: marginStyle, src: this.props.src, alt: this.props.Text, onClick: this.clickHandler });
    }
});
